...
(function (global, factory) {
  ...
  function toNodes(element) {
    return (
      (element &&
        (isNode(element) ? [element] : Array.from(element).filter(isNode))) ||
      []
    );
  }
  ...
  function toWindow(element) {
    ...
    const document = isDocument(element)
      ? element
  ...
  function intersectRect() {
    for (
      var _len = arguments.length, rects = new Array(_len), _key = 0;
      _key < _len;
      _key++
    ) {
  ...
  function attr(element, name, value) {
    ...
    if (isUndefined(value)) {
      var _toNode;
      return (_toNode = toNode(element)) == null
        ? void 0
        : _toNode.getAttribute(name);
    } else {
  ...
  const isContextSelector = memoize((selector) =>
    selector.match(contextSelectorRe)
  );
  ...
  const sanatize = memoize((selector) =>
    selector.replace(contextSanitizeRe, "$1 *")
  );
  ...
  function on() {
    ...
    let [targets, types, selector, listener, useCapture = false] =
      getArgs(args);
  ...
  function once() {
    ...
    const [element, types, selector, listener, useCapture = false, condition] =
      getArgs(args);
    const off = on(
      element,
      types,
  ...
  function getEventPos(e) {
    ...
    const { clientX: x, clientY: y } =
      ((_e$touches = e.touches) == null ? void 0 : _e$touches[0]) ||
      ((_e$changedTouches = e.changedTouches) == null
        ? void 0
        : _e$changedTouches[0]) ||
      e;
  ...
  function send(url, env) {
    return new Promise((resolve, reject) => {
      ...
      on(xhr, "timeout", () =>
        reject(assign(Error("Network Timeout"), { xhr }))
      );
  ...
  function css(element, property, value, priority) {
    ...
    for (const element of elements) {
      if (isString(property)) {
        ...
        priority = value;
        each(property, (value, property) =>
          css(element, property, value, priority)
        );
      }
    }
    return elements[0];
  ...
  function removeClasses(element, cls) {
    attr(element, "class", (value) =>
      (value || "").replace(new RegExp("\\b" + cls + "\\b", "g"), "")
    );
  }

  function replaceClass(element) {
    (arguments.length <= 1 ? undefined : arguments[1]) &&
      removeClass(element, arguments.length <= 1 ? undefined : arguments[1]);
    (arguments.length <= 2 ? undefined : arguments[2]) &&
      addClass(element, arguments.length <= 2 ? undefined : arguments[2]);
  }

  function hasClass(element, cls) {
    [cls] = getClasses(cls);
    return (
      !!cls && toNodes(element).some((node) => node.classList.contains(cls))
    );
  }
  ...
  const parseCalc = memoize(
    (calc) => calc.toString().replace(/\s/g, "").match(calcRe) || []
  );
  const unitRe$1 = /(?:v[hw]|%)$/;
  ...
  function isTag(element, tagName) {
    ...
    return (
      (element == null
        ? void 0
        : (_element$tagName = element.tagName) == null
        ? void 0
        : _element$tagName.toLowerCase()) === tagName.toLowerCase()
    );
  }
  ...
  */
  ...
  function scrolledOver(element, startOffset, endOffset) {
    ...
    const elementOffsetTop =
      offsetPosition(element)[0] - offsetPosition(scrollElement)[0];

    const start = Math.max(0, elementOffsetTop - viewportHeight + startOffset);
    const end = Math.min(
      maxScroll,
      elementOffsetTop + element.offsetHeight - endOffset
    );
  ...
  function scrollParents(element, overflowRe, scrollable) {
    ...
    const fixedIndex = findIndex(
      ancestors,
      (el) => css(el, "position") === "fixed"
    );
    if (~fixedIndex) {
  ...
  function offsetViewport(scrollElement) {
    ...
    for (let [prop, dir, start, end] of [
      ...
      if (!isWindow(getViewport$1(viewportElement))) {
        rect[start] += toFloat(
          css(viewportElement, "border" + ucfirst(start) + "Width")
        );
      }
      rect[prop] = rect[dir] = (
        isWindow(viewportElement)
          ? scrollingElement(viewportElement)
          : viewportElement
      )["client" + ucfirst(prop)];
  ...
  function getViewport$1(scrollElement) {
    return scrollElement === scrollingElement(scrollElement)
      ? window
      : scrollElement;
  }
  ...
  function attachToWithFlip(element, target, options) {
    ...
    for (const [i, [prop, dir, start, end]] of Object.entries(dirs)) {
      ...
      const willFlip =
        !intersectLine(position, targetDim, i) &&
        intersectLine(position, targetDim, 1 - i);

      viewport = getIntersectionArea(
        ...viewports.filter(Boolean).map(offsetViewport)
      );
  ...
  function hooksAPI(UIkit) {
    UIkit.prototype._callHook = function (hook) {
      ...
      (_this$$options$hook = this.$options[hook]) == null
        ? void 0
        : _this$$options$hook.forEach((handler) => handler.call(this));
    };
  ...
  function componentAPI(UIkit) {
    ...
    UIkit.getComponents = (element) =>
      (element == null ? void 0 : element[DATA]) || {};
  ...
  function boot(UIkit) {
    ...
    function applyAttributeMutation(_ref2) {
      ...
      (_UIkit$getComponent = UIkit.getComponent(target, name)) == null
        ? void 0
        : _UIkit$getComponent.$destroy();
    }
  }
      ...
      lazyload(observeTargets, targets) {
        ...
        this.registerObserver(
          observeIntersection(observeTargets, (entries, observer) => {
            for (const el of toNodes(
              isFunction(targets) ? targets() : targets
            )) {
              $$('[loading="lazy"]', el).forEach((el) =>
  ...
  var Togglable = {
    ...
    methods: {
      ...
      _toggle(el, toggled) {
        ...
        $$("[autofocus]", el).some((el) =>
          isVisible(el) ? el.focus() || true : el.blur()
        );
  ...
  function toggleHeight(_ref3) {
    let { isToggled, duration, initProps, hideProps, transition, _toggle } =
      _ref3;
    return (el, show) => {
  ...
  var Accordion = {
    ...
    computed: {
      items: {
        ...
        watch(items, prev) {
          items.forEach((el) =>
            hide($(this.content, el), !hasClass(el, this.clsOpen))
          );
    ...
    methods: {
      toggle(item, animate) {
        ...
        for (const el of items) {
          this.toggleElement(
            el,
            !hasClass(el, this.clsOpen),
            async (el, show) => {
              toggleClass(el, this.clsOpen, show);
              attr($(this.$props.toggle, el), "aria-expanded", show);

              const content = $(
                "" + (el._wrapper ? "> * " : "") + this.content,
                el
              );
  ...
  var Video = {
    ...
    connected() {
      ...
      this.registerObserver(
        observeIntersection(this.$el, () => this.$emit(), {}, false)
      );
    },
  ...
  var Resize = {
    connected() {
      ...
      this.registerObserver(
        observeResize(
          ((_this$$options$resize = this.$options.resizeTargets) == null
            ? void 0
            : _this$$options$resize.call(this)) || this.$el,
          () => this.$emit("resize")
        )
      );
    },
    ...
    methods: {
        ...
        if (active$1) {
          if (delay && active$1.isDelaying) {
            this.showTimer = setTimeout(
              () => matches(target, ":hover") && this.show(),
              10
            );
            return;
          }
      ...
      position() {
        ...
        const maxWidth =
          ...
          (this.boundaryAlign ? 0 : 2 * this.viewportPadding);
  ...
  var formCustom = {
    ...
    update() {
      ...
      const value =
        (_input$files = input.files) != null && _input$files[0]
          ? input.files[0].name
  ...
  function match$1(elements) {
    ...
    return {
      heights: elements.map((el, i) =>
        heights[i].toFixed(2) === max.toFixed(2) ? "" : max
      ),
      elements,
    };
  }
      ...
      read(_ref) {
        ...
        if (this.expand) {
          ...
          if (this.offsetBottom === true) {
            minHeight +=
              " - " + dimensions$1(this.$el.nextElementSibling).height + "px";
            ...
            minHeight +=
              " - " +
              dimensions$1(query(this.offsetBottom, this.$el)).height +
              "px";
          }
  ...
  var SVG = {
    ...
    methods: {
      async getSvg() {
        ...
        return (
          parseSVG(await loadSVG(this.src), this.icon) ||
  ...
  const Search = {
    ...
    beforeConnect() {
      this.icon =
        ...
        parents(this.$el, ".uk-search-large").length
          ? "search-large"
  ...
  const Close = {
    ...
    beforeConnect() {
      this.icon =
        "close-" + (hasClass(this.$el, "uk-close-large") ? "large" : "icon");
    },
  };
    ...
    connected() {
      ...
      this.registerObserver(
        observeIntersection(
          ...
          {
            rootMargin:
              toPx(this.offsetTop, "height") +
              "px " +
              toPx(this.offsetLeft, "width") +
              "px",
          }
        )
      );
  ...
  var Media = {
    ...
    disconnected() {
      ...
      (_this$offMediaObj = this.offMediaObj) == null
        ? void 0
        : _this$offMediaObj.call(this);
    },
  };
        ...
        handler() {
          ...
          if (this.stack) {
            css(
              this.$el,
              "zIndex",
              toFloat(css(this.$el, "zIndex")) + active.length
            );
          }
        ...
        get(_ref5, $el) {
          ...
          if (this.dropContainer !== $el) {
            for (const el of $$("." + clsDrop, this.dropContainer)) {
              ...
              const target =
                (_this$getDropdown = this.getDropdown(el)) == null
                  ? void 0
                  : _this$getDropdown.target;
              if (
                !includes(dropdowns, el) &&
                target &&
                within(target, this.$el)
              ) {
                dropdowns.push(el);
...

      {
        ...
        handler(e) {
          ...
          if (keyCode === keyMap.ESC) {
            ...
            active == null
              ? void 0
              : (_active$target = active.target) == null
              ? void 0
              : _active$target.focus();
          }
...

      {
        ...
        handler(e, _ref11) {
          ...
          if (
            ...
            !this.toggles.some(
              (el) => active.target !== el && matches(el, ":focus")
            )
          ) {
            e.preventDefault();
          }
        ...
        return {
          max: Math.max(
            ...
            height(this.container) -
              (dimensions$1(this.content).height - height(this.$el))
          ),
        };
      },
    ...
    computed: {
      elements: {
        ...
        watch(elements, prev) {
          if (this.hidden) {
            css(
              filter$1(elements, ":not(." + this.inViewClass + ")"),
              "visibility",
              "hidden"
            );
          }
    ...
    update: [
      {
        read(_ref2, types) {
          ...
          if (
            position === "bottom" &&
            (height$1 < windowHeight || this.overflowFlip)
          ) {
        ...
        write(data, types) {
          ...
          if (
            ...
            if (this.animation && scroll > topOffset) {
              Animation.cancel(this.$el);
              Animation.out(this.$el, this.animation).then(
                () => this.hide(),
                noop
              );
            } else {
...

      {
        ...
        el() {
          return this.connects.concat(
            this.itemNav ? queryAll(this.itemNav, this.$el) : []
          );
        },
  ...
  function slide(action, target, duration) {
    return new Promise((resolve) =>
      requestAnimationFrame(() => {
        ...
        // Find new nodes
        nodes = nodes.concat(
          children(target).filter((el) => !includes(nodes, el))
        );
        ...
        Promise.resolve().then(() => {
          ...
          const [propsTo, propsFrom] = getTransitionProps(
            target,
            nodes,
            currentProps
          );
        ...
        watch() {
          ...
          if (this.selActive !== false) {
            ...
            this.toggles.forEach((el) =>
              toggleClass(el, this.cls, includes(actives, el))
            );
          }
        },
  ...
  function isEqualState(stateA, stateB) {
    return ["filter", "sort"].every((prop) =>
      isEqual(stateA[prop], stateB[prop])
    );
  }
  ...
  function isEqualList(listA, listB) {
    return (
      listA.length === listB.length && listA.every((el) => ~listB.indexOf(el))
    );
  }
  ...
  var SliderNav = {
    ...
    update: {
      write() {
        if (this.nav && this.length !== this.nav.children.length) {
          html(
            ...
            this.slides
              .map(
                (_, i) =>
                  "<li " + this.attrItem + '="' + i + '"><a href></a></li>'
              )
              .join("")
          );
        }
        ...
        this.navItems
          .concat(this.nav)
          .forEach((el) => el && (el.hidden = !this.maxIndex));
        ...
        async handler(_, item) {
          ...
          if (
            ...
            try {
              ...
              this.setItem(
                item,
                createEl("img", { src, width, height, alt, ...attrs })
              );
            } catch (e) {
              this.setError(item);
            }
  ...
  var lightbox = {
    ...
    methods: {
      show(index) {
        ...
        this.panel =
          this.panel ||
          this.$create("lightboxPanel", { ...this.$props, items });
  ...
  function transformFn(prop, el, stops) {
    let unit =
      getUnit(stops) || { x: "px", y: "px", rotate: "deg" }[prop] || "";
    let transformFn;

    if (prop === "x" || prop === "y") {
      prop = "translate" + ucfirst(prop);
      transformFn = (stop) =>
        toFloat(toFloat(stop).toFixed(unit === "px" ? 0 : 6));
    } else if (prop === "scale") {
      unit = "";
      transformFn = (stop) =>
  ...
  function setBackgroundPosFn(prop, stops, pos) {
    return function (css, percent) {
      css["background-position-" + prop] =
        "calc(" + pos + " + " + getValue(stops, percent) + "px)";
    };
  }

  const dimensions = {};
  function getBackgroundImageDimensions(el) {
    const src = css(el, "backgroundImage").replace(
      /^none|url\(["']?(.+?)["']?\)$/,
      "$1"
    );
  ...
  var parallax = {
    ...
    update: {
      read(_ref4, types) {
        ...
        const prev = percent;
        percent = ease(
          scrolledOver(this.target, this.start, this.end),
          this.easing
        );
  ...
  function Transitioner(prev, next, dir, _ref) {
    ...
    return {
      ...
      getItemIn(out) {
        ...
        return nextActives[
          findIndex(nextActives, (el) => !includes(actives, el))
        ];
      },
  ...
  function getWidth(list) {
    return children(list).reduce(
      (right, el) => dimensions$1(el).width + right,
      0
    );
  }
  ...
  function getElLeft(el, list) {
    return (
      (el &&
        (position(el).left +
          (isRtl ? dimensions$1(el).width - dimensions$1(list).width : 0)) *
          (isRtl ? -1 : 1)) ||
      0
    );
  }
  ...
  function inView(list, listLeft) {
    ...
    return children(list).filter((slide) => {
      ...
      const slideRight =
        slideLeft + Math.min(dimensions$1(slide).width, listWidth);
  ...
  var slider = {
      ...
      sets(_ref2) {
        ...
        for (let i = 0; i < this.slides.length; i++) {
          ...
          if (this.center) {
            if (
              left < width / 2 &&
              left + slideWidth + dimensions$1(this.slides[+i + 1]).width / 2 >
                width / 2
            ) {
              sets.push(+i);
    ...
    methods: {
      reorder() {
        ...
        let width =
          dimensions$1(this.list).width / 2 - dimensions$1(next).width / 2;
        let j = 0;
    ...
    events: [
      {
        ...
        handler(_ref) {
          let {
            type,
            detail: { percent, duration, timing, dir },
          } = _ref;
          fastdom.read(() => {
  ...
  var sortable = {
    ...
    methods: {
      ...
      insert(element, target) {
        ...
        const insert = () =>
          target ? before(target, element) : append(this.target, element);
  ...
  function findTarget(items, point) {
    return items[
      findIndex(items, (item) =>
        pointInRect(point, item.getBoundingClientRect())
      )
    ];
  }
